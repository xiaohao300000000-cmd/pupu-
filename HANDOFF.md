...
2. 商品候选、替换、偏好、库存和菜品输入的非外部依赖代码已补齐；真实飞书发送与客户端交互必须等待全新的飞书应用和凭证。
3. 用户提供可调用 signer 命令和真实向量后，把命令写入本地 `PUPUSGN_BLACKBOX_CMD`，再实现门店、商品和购物车 Gateway。
4. 历史复购需要真实订单列表和订单详情；受保护 Connector 可用后再实现并验收，不能用本地历史或 Mock 冒充。
5. 受保护 Connector 可用后，按文档跑通“帮我买牛奶”的真实最小闭环，再连接飞书新应用的真实消息发送与回调监听。
6. 最终验收必须由用户在同账号朴朴手机 App 中确认商品和数量一致。
